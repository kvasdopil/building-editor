...
- Existing relation geometry is written through its member ways, never by replacing the assembled relation geometry. Parsed relation features retain every loaded member way's coordinates, node ids, direction, version and tags. Existing boundary nodes may move, and new vertices may be inserted between consecutive surviving nodes of either a closed member or one of several open members that assemble a ring. The relation and its member list remain unchanged unless its tags also changed. Removing or reordering an existing boundary node is normally unsupported because the owning member path can no longer be inferred safely. A closed member has a narrow retained-node exception: an old boundary node may leave the retained ring only if Add part or Slice keeps it on a created part in the same building group, and at least three distinct original anchors survive in order. Add part applies this only to outer rings; Slice may transfer outer or inner boundary nodes to the new sibling fragment. The original closed member still owns the retained ring, while transferred entrance nodes retain their identity and tags on the sibling. Open-member chains and removals without that retained-node evidence remain blocked.
- Polygon and hole array positions carry no OSM identity. When geometry processing reorders a relation's polygons or inner rings, submission pairs each edited ring to exactly one source ring by role and the cyclic order of its surviving OSM node anchors. Zero or multiple matches remain an error rather than an arbitrary choice.
...
| `overlapping-volumes`                        | Two parts overlap in plan **and** in their `[min_height, height]` range. 2D overlap is explicitly allowed; overlapping 3D volumes are what the wiki says to avoid. A ground-level shorter part beneath a strictly taller part can offer **Fix** to raise the taller part's `min_height` to the supporting roof, as described below.                                                                                                                                                                                 |
...
The tower minimum-height fix uses the **lowest modeled roof height within the shared footprint**,
not the lower feature's global peak or eaves. It follows the same roof profile, direction,
orientation and inherited parent frame as the 3D view, including profile breaks and valleys
between footprint corners. Flat or unmodeled roofs retain the effective top-height behavior.
Only an increase of more than 1 cm is offered; it edits the tower alone and rebuilds the checks.
A flat tower base meeting the low side of a curved roof necessarily intersects its higher portions,
so the range-based overlap warning can remain for review, but never offers the same fix again.

The [tower roof regression tests](../../../scripts/lib/tower-roof.test.mjs) verify flat and curved
support, roof orientation/direction, inherited frames, holes, disjoint overlaps and repeat-fix
prevention. Run them with `node --test scripts/lib/tower-roof.test.mjs`.
