# Domain Specs - Index

Status: Draft (2026-03-26)

This section holds business-domain rules, workflows, and invariants that cut across implementation details. Use it when a topic is more about product behavior than repository structure.

Related documents:

- [Specifications index](../index.md): Parent catalog for all normative docs. Read this to navigate between project, domain, and testing specs.
- [Repository structure](../../structure.md): High-level repo layout and section boundaries. Read this when deciding whether a new concept belongs in domain specs or elsewhere.

- [building-explorer.md](building-explorer.md) — Map + 3D building viewer: interaction spec, height rules, and the Overture-over-Overpass data decision. Read before changing selection or extrusion logic.
