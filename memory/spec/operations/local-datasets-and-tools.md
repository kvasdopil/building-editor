...
- IGN LiDAR HD is not a local dataset. `/api/ign` resolves metropolitan EPSG:2154 COPC files through
  IGN's public WFS tile index, range-reads the required octree nodes, and stores assembled LDR1 tiles
  under `.cache/ign`. Source id three identifies those tiles. No environment variable is required.
