...
- The 3D view draws the cloud as coloured dots around the selected building, in the same local frame as the extruded solids. Nothing is inferred from it yet: no suggested tags, no per-part heights.
...
- Dot heights are RH2000 levels minus one ground level for the whole neighborhood, taken as the median of nearby ground returns. On a slope the far side of the view sits slightly off — the same simplification the extruded buildings already make by standing every building on zero.
