...
- **Multipolygon outlines**: relation footprints retain their member-way node identities. Reshape, Add node, wall handles, Slice and Add part can therefore move existing corners and insert new outline nodes into either closed members or rings assembled from open members, without rewriting the relation topology. Unsupported node removal or reordering remains blocked; Add part can move a boundary stretch into a shared internal wall when one closed member owns the outer ring and the old nodes survive on the new part.
  Geometry edits now check that mapping before they are saved. Unsupported edits leave the previous
  state intact. Compound geometry changes must be undone together; the Changes sidebar disables
  parent-only or part-only discard when it would split an applied edit.
- **3D view**: selecting a building or `building:part` opens a side panel with a [Three.js](https://threejs.org/) extrusion on [Mapterhorn](https://mapterhorn.com/) z13 terrain. The selected building's lowest terrain sample is scene zero; neighbors use their own footprint minima, so slopes remain visible without tilting a building. Clicking any visible building or part extrusion selects, highlights, inspects, and frames it; orbit drags do not change selection. A selected part retains its parent and sibling parts for context. The initial bearing matches the map at the moment the entity is selected, then zoom (scroll) and rotation (drag) are independent via OrbitControls. Adjacent buildings within 80 m are drawn in gray for context (capped at 60, nearest first).
...
- **Inspector**: below the 3D view the panel starts with the selected feature id, then lists every OSM tag on the selected building or part, with frequently edited height and roof properties grouped first. There is no separate building/part header or panel-level close/revert toolbar. A part's `building:part` row includes a parent control. Hovering `height`, `building:levels`, `min_height`, or `building:min_level` reveals a pencil; its modal saves with Enter and cancels with Escape. Horizontal handles adjust `height`, `min_height`, and `roof:height` in 0.5 m steps. Shaped roofs use a type select; gabled, gambrel, and round roofs add a `roof:orientation` select, while skillion roofs add a compass drag handle beside a clickable `roof:direction` degree value. Accepted values update the map and 3D view immediately. Pending edits are stored against that exact OSM entity; LOD1 advice remains building-outline only.
...
- **Complex shaped roofs**: `roof:shape=hipped` uses an interior straight skeleton to turn the complete footprint into one equal-pitch roof. Concave L-, H- and T-shaped outlines form their own connected hips, ridges and valleys instead of being split into rectangular roofs; invalid skeletons fall back to the pyramidal surface rather than disappearing. `gabled` and `gambrel` build on the same skeleton once an outline turns inward, with the walls that would have carried a hip end suppressed, so every wing gets a ridge running out to its own gable wall; convex outlines and rectangles keep the bounding-rectangle construction byte for byte.
- **Mansard roofs**: `roof:shape=mansard` uses that same complete-footprint straight skeleton, with a hard break at 30% of each face's run and 60% of its rise. The result has steep lower panels and shallow upper panels across complex outlines and holes, with the same pyramidal fallback when skeleton geometry is unavailable.
...
## French national laser data, on demand

Metropolitan France uses IGN's open **LiDAR HD** coverage, including Paris and the Eiffel Tower. It
needs no account and no import. `/api/ign/tile/{z}/{x}/{y}` queries IGN's public WFS kilometre-tile
index, range-reads only the required nodes from the returned classified COPC files, converts
Lambert-93 (EPSG:2154) to WGS84, and caches the assembled browser tile under `.cache/ign`. The app
reads whole COPC levels toward four points/m², caps each browser tile at 500,000 points, and labels
it `IGN LiDAR HD`; the source guarantees at least 10 acquisition pulses/m². The source has
classification rather than RGB, so Color mode uses stable ground, vegetation, building, water, and
bridge colours. See
[ADR 0009](memory/adr/0009-ign-lidar-hd-read-on-demand.md).
...
bow-tie. A consecutive duplicated node also offers **Fix** when one redundant local corner can be
removed while leaving a simple ring.
Highlights:
...
yarn test        # node --test over the offline unit tests
yarn test:lidar  # the LiDAR reader tests, which reach upstream
yarn build       # production build
yarn advice      # roof measurements for a building, from the terminal
```

`yarn test` runs the app's own modules through `scripts/lib/ts-hooks.mjs`, the same loader
`yarn advice` uses. `scripts/lib/roof-skeleton.test.mjs` defines `window` and `self` before
importing, because the straight-skeleton engine is a browser Wasm build and will not load under
Node without them — which is also why the roof advice CLI never offers the skeleton shapes.
