...
- **LiDAR**: with a building selected, toggle a mutually exclusive top-down point-cloud mode on the map. It reuses the same merged municipal/national cloud as the 3D view but sends only longitude and latitude to a GPU point layer—survey height never affects position—so the view remains flat XY beneath building and part boundaries.
