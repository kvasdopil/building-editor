 ...
 * and `normal` colours by how steeply each link rises: violet lying flat
 * through to red standing vertical.
 */
export type LidarColourMode = "colour" | "height" | "normal";
...
export class LidarMapLayer implements CustomLayerInterface {
  ...
  private heightRangeLocation: WebGLUniformLocation | null = null;
  ...
  /** Lowest and highest point currently in view, the ends of the ramp. */
  ...
  private refitRamp(): void {
    if (this.visible && this.mode === "height") this.refreshHeightRange();
  }
  ...
  private refreshHeightRange(): void {
    ...
    this.heightRange = [low, high - low > 0.01 ? high : low + 0.01];
    this.map?.triggerRepaint();
  }
  ...
  onAdd(map: MaplibreMap, gl: WebGL2RenderingContext): void {
    ...
    const vertex = compileShader(
      ...
      // 0 = survey colour, 1 = height ramp, 2 = link angle.
      uniform int u_mode;
      ...
      void main() {
        ...
        if (u_mode == 1) {
          float span = max(u_height_range.y - u_height_range.x, 0.01);
          v_colour = srgb_to_linear(violet_to_red((a_height - u_height_range.x) / span));
        } else if (u_mode == 2) {
    ...
    this.heightRangeLocation = gl.getUniformLocation(program, "u_height_range");
  ...
  render(gl: WebGL2RenderingContext, options: CustomRenderMethodInput): void {
    ...
    gl.uniform2f(this.heightRangeLocation, this.heightRange[0], this.heightRange[1]);
  ...
  private upload(): void {
    ...
    if (!this.heightBuffer || !this.inclineBuffer) return;
    gl.bindBuffer(gl.ARRAY_BUFFER, this.positionBuffer);
